...
Lite MUST 在全部固定禁止包均未安装时导入并运行应用生命周期、Scheduler、Command、MessageChain、Monitor、TransferChain、PluginManager、保留存储、元数据、整理、通知、媒体服务器和管理员认证链。类型注解、公共 Schema、模块 `__init__` 或历史禁用方法 MUST NOT 在模块顶层恢复禁止依赖。
