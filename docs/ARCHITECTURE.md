...
API 路由门控已经消费该配置：主 API 和独立兼容接口必须先检查能力，再导入对应端点模块。PT 用户认证运行链路也已完成首批裁剪：`AUTH_SITE` 不再生效，管理员认证上下文固定为本地等级 1，插件等级 2、3、99 不再通过在线认证或私钥提升，启动阶段不再初始化站点认证/索引资源，Scheduler 不再注册 CookieCloud 同步、用户认证检查和站点数据刷新任务，也不清理历史站点用户数据。模块扫描、其余启动服务与后台任务、消息命令、依赖和前端门控仍由后续 OpenSpec 分阶段实施，不得因当前裁剪而宣称其他边界已经完成。
...
现有 `ModuleHelper.load()` 会在过滤对象前导入所有模块，不能直接作为 Lite 的导入门控。Lite 必须在模块包导入前完成筛选。
